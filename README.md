...
працівників у Європі: Головна, сторінка партнера з вакансіями (пошук +
фільтр за категорією) та сторінка контактів із формою заявки.
...
Без цього кроку сторінка партнера й блок партнерів на Головній коректно
покажуть **retry-блок** ("не вдалося завантажити") — це очікувана поведінка
асинхронного шару, а не помилка коду.
...
app/[locale]/                 # усі сторінки під локаллю (uk за замовч., en, pl)
  ...
  page.tsx                     # Home
  partners/[slug]/page.tsx     # сторінка партнера (динамічна)
  contact/page.tsx             # /контакти (uk) / /contact (en) / /kontakt (pl)

components/
  layout/     Header, Footer, LocaleSwitcher, MobileNav
  home/       Hero, CategoryGrid, FeaturedPartnersSection(+Skeleton), EmployerCtaSection
  partners/   PartnerHeader, PartnerJobsBoard, JobSearchInput, CategoryFilter,
              JobList, JobCard, JobListSkeleton
  contact/    ContactForm
...
lib/
  ...
  filterJobs.ts    чиста функція пошук+категорія
...
data/        categories.ts, locations.ts, constants.ts (фіксовані таксономії)
...
**Список вакансій без зайвих ре-рендерів:** вакансії партнера завантажуються
один раз (`useAsync`, по `partnerId`), пошук і категорія фільтрують
виключно на клієнті. Сирий стан кожного натискання клавіші живе всередині
`JobSearchInput` (не в батьківському `PartnerJobsBoard`) — `onDebouncedChange`
стабільний (`useCallback`), тож `memo(JobSearchInput)` не ре-рендериться
через активність батька; ре-рендер від символу лишається в цьому листовому
компоненті й не каскадує далі. `filterJobs` використовує лише
`Array.prototype.filter` (без map/clone) — job-об'єкти, що пройшли фільтр,
зберігають referential identity, тож `React.memo(JobCard)` не
перерендерюється, якщо конкретна вакансія не змінилась.

## Unit-тести

`npm run test:coverage` — 37 тестів, **~97% покриття** логіки, яку оцінює
...
2. **Швидший пошук вакансії** — категорійні чіпи на Головній ведуть одразу
   на сторінку партнера з попередньо застосованим фільтром
   (`?category=...`), а не лише на порожній список, який ще треба
   відфільтрувати вручну.
...
- **Локалізація (uk / en / pl) — фіча понад бриф.** Не вимагалась завданням,
  додана як ініціатива: next-intl, локалізовані шляхи (`/контакти` для uk,
  `/contact` для en, `/kontakt` для pl — буквальна вимога брифу щодо
  `/контакти` збережена для дефолтної локалі), переклад контенту вакансій/
  партнерів у самій БД (jsonb-поля `{uk, en, pl}`), а не лише інтерфейсу.
- **Один партнер за замовчуванням для глибоких посилань.** Бриф описує
  єдиний шаблон сторінки партнера (без індексу партнерів), тож "Знайти
  роботу" в хедері та категорійні чіпи на Головній ведуть на конкретного
  партнера (`data/constants.ts → DEFAULT_PARTNER_SLUG`), а не на список усіх
  партнерів, якого в брифі немає.
